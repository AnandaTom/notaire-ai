---
name: review
description: MANDATORY after writing or modifying any Python script. Reviews code with fresh eyes for correctness and effectiveness (2-pass). Must be called before /document and before committing. Auto-invoke after any code creation or modification — do not wait for user to ask.
...
3. **Spawn the reviewer subagent** using the Task tool:
   ...
   ```
   Pass it the full file contents and a clear description of what the code is supposed to do. The reviewer agent prompt lives at `~/.claude/agents/reviewer.md` — it performs a 2-pass review (correctness then effectiveness).
