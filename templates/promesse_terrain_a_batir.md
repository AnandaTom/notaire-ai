...
{% if bien.description %}{{ bien.description }}{% endif %}
...
{% if bien.nature_terrain %}
**Nature** : {{ bien.nature_terrain | default("Terrain à bâtir") }}
{% endif %}

{% if bien.surface_terrain %}
**Surface** : {{ bien.surface_terrain }} m² d'après le document d'arpentage{% if bien.bornage %} et le procès-verbal de bornage{% endif %}
{% endif %}

{% if bien.lotissement %}
### Lotissement
Le terrain fait partie du lotissement dénommé **"{{ bien.lotissement.nom }}"**, autorisé par {{ bien.lotissement.autorite | default("le Maire") }} de {{ bien.lotissement.commune | default(bien.adresse.ville) }} suivant arrêté en date du {{ bien.lotissement.date_permis | format_date }}, sous le numéro {{ bien.lotissement.numero_permis }}.

{% if bien.lotissement.cahier_charges %}
Un cahier des charges du lotissement a été établi par {{ bien.lotissement.cahier_charges.auteur | default("le lotisseur") }}{% if bien.lotissement.cahier_charges.date %} le {{ bien.lotissement.cahier_charges.date | format_date }}{% endif %}.
Le **BENEFICIAIRE** déclare avoir pris connaissance du cahier des charges du lotissement et s'engage à en respecter les prescriptions.
{% endif %}

{% if bien.lotissement.asl %}
Il existe une Association Syndicale Libre (ASL) dénommée "{{ bien.lotissement.asl.nom }}" ayant pour objet {{ bien.lotissement.asl.objet | default("la gestion des équipements communs du lotissement") }}.
{% endif %}
{% endif %}

{% if bien.bornage %}
### Bornage
Un procès-verbal de bornage a été dressé par {{ bien.bornage.geometre }}{% if bien.bornage.date %}, en date du {{ bien.bornage.date | format_date }}{% endif %}.
{% else %}
### Obligation d'information sur le bornage
Conformément aux articles L. 115-4 et suivants du Code de l'urbanisme, lorsque le terrain fait partie d'un lotissement, sa surface est garantie.
Le **PROMETTANT** déclare que le bornage du terrain {% if bien.bornage_effectue %}a été réalisé{% else %}n'a pas encore été réalisé et s'engage à le faire établir avant la réitération{% endif %}.
{% endif %}

Tel que le **TERRAIN** existe, avec tous droits y attachés, sans aucune exception ni réserve.

# ABSENCE DE MEUBLES ET OBJETS MOBILIERS
La présente promesse porte exclusivement sur le terrain nu, aucun meuble ou objet mobilier n'est compris dans la vente.
...
**Annexe n°2 : Plans du terrain**

## Contenance du terrain
Le terrain objet des présentes a une contenance de **{{ bien.surface_terrain | default("_____") }} m²** d'après le titre de propriété{% if bien.bornage %} et le procès-verbal de bornage établi par {{ bien.bornage.geometre }}{% if bien.bornage.date %} le {{ bien.bornage.date | format_date }}{% endif %}{% endif %}.

{% if bien.tolerance_superficie is defined %}
La présente vente est consentie sans garantie de contenance, toute différence en plus ou en moins entre la contenance réelle et celle indiquée aux présentes, excédât-elle un vingtième, fera profit ou perte pour le **BENEFICIAIRE** qui renonce à tout recours, à ce sujet, contre le **PROMETTANT**.
{% else %}
En application de l'article L. 115-5 du Code de l'urbanisme, si la superficie réelle est inférieure de plus d'un vingtième à celle exprimée dans l'acte, le **BENEFICIAIRE** pourra exiger une diminution du prix proportionnelle, ou la résolution de la vente. L'action doit être intentée dans le mois de la livraison.
{% endif %}
...
| Réseau | Statut | Distance |
| :---- | :---- | :---- |
{% for raccord in bien.viabilisation.raccordements %}
| {{ raccord.reseau }} | {{ raccord.statut | default("disponible") }} | {{ raccord.distance | default("-") }} |
{% endfor %}
{% endif %}

{% if bien.viabilisation.cout_estimatif %}
**Coût estimatif de viabilisation** : {{ bien.viabilisation.cout_estimatif | format_nombre }} EUR
{% endif %}
{% endif %}
...
Un certificat d'urbanisme {{ bien.constructibilite.cu_type | default("opérationnel") }} a été délivré le {{ bien.constructibilite.cu_date | format_date }} sous la référence {{ bien.constructibilite.cu_reference }}.
{% endif %}

{% if bien.constructibilite.zone_plu %}
Le terrain se situe en zone **{{ bien.constructibilite.zone_plu }}** du Plan Local d'Urbanisme.
{% endif %}
...
# Dispositions relatives à la construction
{% if terrain_construction.etude_geotechnique %}
## Étude géotechnique
Conformément à l'article L. 132-6 du Code de la construction et de l'habitation, une étude géotechnique préalable a été réalisée par {{ terrain_construction.etude_geotechnique.cabinet }} le {{ terrain_construction.etude_geotechnique.date | format_date }}.
{% else %}
## Étude géotechnique
Conformément à l'article L. 132-6 du Code de la construction et de l'habitation, lorsque le terrain est situé dans une zone d'exposition au phénomène de mouvement de terrain différentiel, une étude géotechnique est fournie par le vendeur.
{% endif %}

{% if terrain_construction.raccordement_reseaux %}
## Raccordement aux réseaux
{% for reseau in terrain_construction.raccordement_reseaux %}
- **{{ reseau.type }}** : {{ reseau.statut }}{% if reseau.distance %} (à {{ reseau.distance }}){% endif %}{% if reseau.cout %} — Coût estimé : {{ reseau.cout | format_nombre }} EUR{% endif %}
{% endfor %}
{% endif %}

{% if terrain_construction.ccmi %}
## Contrat de construction de maison individuelle
Le **BENEFICIAIRE** déclare avoir l'intention de faire construire une maison individuelle sur le terrain{% if terrain_construction.ccmi.constructeur %}, par la société {{ terrain_construction.ccmi.constructeur }}{% endif %}.
{% if terrain_construction.ccmi.garantie_livraison %}
La garantie de livraison à prix et délais convenus sera fournie par {{ terrain_construction.ccmi.garantie_livraison }}.
{% endif %}
{% endif %}
{% endif %}
...
{% if taxe_terrain_constructible.article_1529 %}
## Taxe communale (article 1529 du CGI)
Le terrain, classé dans une zone urbaine ou à urbaniser du Plan Local d'Urbanisme, est susceptible d'être soumis à la taxe forfaitaire communale prévue par l'article 1529 du Code Général des Impôts.
{% endif %}

{% if taxe_terrain_constructible.article_1605_nonies %}
## Taxe nationale (article 1605 nonies du CGI)
Le terrain est susceptible d'être soumis à la taxe sur la cession de terrains nus devenus constructibles prévue par l'article 1605 nonies du Code Général des Impôts.
{% endif %}
{% endif %}

## Information du bénéficiaire sur sa situation
Le **BENEFICIAIRE** déclare, tenant compte de la situation qui sera la sienne à la réalisation de la vente :
