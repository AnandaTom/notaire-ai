...
**-** Les **"BIENS"** désigneront le bien et les droits immobiliers objet de la présente promesse de vente.
...
{% if bien.description_ensemble %}{{ bien.description_ensemble }}{% endif %}
...
**Surface du terrain** : {{ bien.surface_terrain }} m²
{% endif %}

{% if bien.dependances %}
**Dépendances** :
{% for dep in bien.dependances %}
- {{ dep.nature }}{% if dep.surface %} ({{ dep.surface }} m²){% endif %}
{% endfor %}
{% endif %}

{% if bien.construction %}
**Construction** :
- Date de construction : {{ bien.construction.date | default("Non renseignée") }}
{% if bien.construction.permis %}
- Permis de construire : {{ bien.construction.permis }}
{% endif %}
{% if bien.construction.conformite %}
- Attestation de conformité : {{ bien.construction.conformite }}
{% endif %}
{% endif %}
...
{% if bien.plans_annexes %}
{% for plan in bien.plans_annexes %}
- {{ plan }} ;
{% endfor %}
{% endif %}

**Annexe n°2 : Plans du bien**

## Contenance du terrain et des constructions
{% if bien.surface_terrain %}
Le terrain objet des présentes a une contenance de **{{ bien.surface_terrain }} m²** d'après le titre de propriété{% if bien.bornage %} et le procès-verbal de bornage établi par {{ bien.bornage.geometre }}{% if bien.bornage.date %} le {{ bien.bornage.date | format_date }}{% endif %}{% endif %}.
...
{% if bien.surface_habitable %}
La surface habitable des constructions est de **{{ bien.surface_habitable }} m²**.
{% endif %}

{% if bien.bornage %}
**Annexe n°3 : Procès-verbal de bornage**
{% endif %}

{% if bien.assainissement %}
## Assainissement
{% if bien.assainissement.type == "collectif" %}
Le bien est raccordé au réseau d'assainissement collectif.
{% elif bien.assainissement.type == "individuel" %}
Le bien dispose d'un système d'assainissement individuel ({{ bien.assainissement.details | default("fosse septique") }}).
{% if bien.assainissement.diagnostic %}
Un diagnostic de l'installation d'assainissement non collectif a été établi le {{ bien.assainissement.diagnostic.date | format_date }} par {{ bien.assainissement.diagnostic.prestataire | default("le service compétent") }}.
{% if bien.assainissement.diagnostic.conforme is defined %}
L'installation est déclarée {% if bien.assainissement.diagnostic.conforme %}conforme{% else %}non conforme. Des travaux de mise en conformité seront à la charge de {{ bien.assainissement.travaux_charge | default("l'acquéreur") }}{% endif %}.
{% endif %}
{% endif %}
{% else %}
Le **PROMETTANT** déclare que le bien est raccordé au réseau d'assainissement {{ bien.assainissement.type | default("collectif") }}.
{% endif %}
{% endif %}
