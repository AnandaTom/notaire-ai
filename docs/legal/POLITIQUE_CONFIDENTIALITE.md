# Politique de Confidentialité - Notomai

> **Information sur le traitement des données personnelles**
> Conformément au Règlement (UE) 2016/679 (RGPD)
>
> Date d'entrée en vigueur : 24 janvier 2026
> Dernière mise à jour : 24 janvier 2026
> Version : 1.0

---

## 1. Identité du Responsable de Traitement

Le responsable du traitement de vos données personnelles est l'**étude notariale** que vous avez contactée pour votre dossier.

Notomai agit en qualité de **sous-traitant technique** pour le compte de l'étude notariale.

| Information | Détail |
|-------------|--------|
| Sous-traitant | Notomai - Agence IA Automatisation |
| Représentant | Tom Randeau, Co-Founder |
| Adresse | 2B chemin des Garennes, 69260 Charbonnières-les-Bains |
| Contact | contact@notomai.fr |
| DPO du notariat | ADNOV - 95 Avenue des Logissons, 13770 VENELLES - dpo@groupeadsn.fr |

---

## 2. Données Collectées

Dans le cadre de votre dossier notarial, les données suivantes peuvent être collectées :

### 2.1 Données d'identification

- Nom et prénoms
- Date et lieu de naissance
- Nationalité
- Adresse postale
- Adresse email
- Numéro de téléphone
- Profession
- Numéro de pièce d'identité

### 2.2 Données relatives à la situation personnelle

- Situation matrimoniale (célibataire, marié, pacsé, divorcé, veuf)
- Régime matrimonial
- Nom du conjoint/partenaire le cas échéant

### 2.3 Données relatives au dossier

- Nature de l'opération (vente, achat, succession, etc.)
- Désignation des biens concernés
- Montants et conditions financières
- Informations bancaires pour les règlements

---

## 3. Finalités du Traitement

Vos données sont traitées pour les finalités suivantes :

| Finalité | Base légale |
|----------|-------------|
| Établissement de l'acte authentique | Exécution du contrat + Mission de service public |
| Vérifications légales obligatoires | Obligation légale |
| Conservation des minutes | Obligation légale (75 ans) |
| Formalités administratives (SPF, cadastre) | Obligation légale |
| Facturation et comptabilité | Obligation légale |
| Gestion de la relation client | Intérêt légitime |

---

## 4. Destinataires des Données

Vos données peuvent être communiquées à :

### 4.1 Destinataires obligatoires (base légale)

- Service de la Publicité Foncière
- Cadastre
- Direction Générale des Finances Publiques
- CRPCEN (Caisse de Retraite et de Prévoyance des Clercs et Employés de Notaires)

### 4.2 Destinataires contractuels

- Autres notaires intervenant au dossier
- Banques (pour les prêts et règlements)
- Experts (géomètres, diagnostiqueurs)

### 4.3 Sous-traitants techniques

| Sous-traitant | Rôle | Localisation |
|---------------|------|--------------|
| Notomai | Assistance à la rédaction d'actes | France/UE |
| Supabase | Hébergement sécurisé des données | Union Européenne (Francfort) |
| Anthropic | Assistant IA pour l'aide à la rédaction (chatbot) | États-Unis |

Tous les sous-traitants sont contractuellement tenus aux mêmes obligations de protection des données.

> **Concernant Anthropic** : Lors de l'utilisation du chatbot d'assistance, les échanges textuels peuvent être transmis à l'API Anthropic (Claude) pour générer des réponses. Anthropic s'engage contractuellement à ne pas utiliser ces données pour l'entraînement de ses modèles et à ne pas les conserver au-delà de 30 jours. Un module d'anonymisation est en cours d'intégration pour supprimer les données personnelles avant transmission.

---

## 5. Transferts de Données

**Vos données sont stockées exclusivement dans l'Union Européenne** (région Francfort).

**Exception** : Les échanges avec le chatbot d'assistance sont transmis à Anthropic (États-Unis) pour le traitement par intelligence artificielle, dans le cadre des Clauses Contractuelles Types (SCCs) de la Commission Européenne. Un module d'anonymisation est en cours de déploiement pour minimiser les données personnelles transmises.

Aucun transfert de données personnelles n'est effectué vers des pays tiers hors UE, sauf obligation légale.

---

## 6. Durée de Conservation

| Type de données | Durée | Justification |
|-----------------|-------|---------------|
| Minutes notariales | 75 ans | Obligation légale |
| Annexes des actes | 30 ans | Obligation légale |
| Données comptables | 10 ans | Obligation légale |
| Données de gestion | Durée de la relation + 5 ans | Prescription civile |

À l'issue de ces durées, les données sont :
- Soit archivées définitivement (minutes)
- Soit anonymisées ou supprimées

---

## 7. Sécurité des Données

Nous mettons en oeuvre des mesures techniques et organisationnelles appropriées :

| Mesure | Description |
|--------|-------------|
| **Chiffrement** | AES-256 au repos, TLS 1.3 en transit |
| **Contrôle d'accès** | Isolation stricte par étude notariale |
| **Authentification** | Clés API sécurisées et hashées |
| **Traçabilité** | Journalisation de toutes les actions |
| **Sauvegarde** | Copies automatiques et sécurisées |
| **Pseudonymisation** | Hachage des noms pour les recherches |

---

## 8. Vos Droits

Conformément au RGPD, vous disposez des droits suivants :

### 8.1 Droit d'accès (Article 15)

Vous pouvez obtenir confirmation que vos données sont traitées et en recevoir une copie.

### 8.2 Droit de rectification (Article 16)

Vous pouvez demander la correction de données inexactes ou incomplètes.

### 8.3 Droit à l'effacement (Article 17)

Vous pouvez demander la suppression de vos données, sous réserve des obligations légales de conservation.

> **Important** : Pour les actes authentiques, seule l'anonymisation est possible après les délais légaux. Les minutes notariales doivent être conservées 75 ans.

### 8.4 Droit à la portabilité (Article 20)

Vous pouvez recevoir vos données dans un format structuré et lisible par machine.

### 8.5 Droit d'opposition (Article 21)

Vous pouvez vous opposer au traitement de vos données pour des motifs légitimes, sauf obligations légales.

### 8.6 Droit de limitation (Article 18)

Vous pouvez demander la limitation du traitement dans certains cas.

---

## 9. Comment Exercer vos Droits

### 9.1 Auprès du notaire

Contactez directement l'étude notariale en charge de votre dossier. Votre demande sera traitée sous **1 mois** (prolongeable à 3 mois si complexe).

### 9.2 Auprès du DPO

Pour toute question sur la protection de vos données dans le notariat :

- **ADNOV** (DPO mutualisé du notariat)
- Email : dpo@adnov.fr
- Adresse : [Adresse ADNOV]

### 9.3 Réclamation

Si vous estimez que vos droits ne sont pas respectés, vous pouvez introduire une réclamation auprès de :

**Commission Nationale de l'Informatique et des Libertés (CNIL)**
- Site web : www.cnil.fr
- Adresse : 3 Place de Fontenoy - TSA 80715 - 75334 Paris Cedex 07

---

## 10. Cookies et Traceurs

L'application Notomai n'utilise **pas de cookies** ni de traceurs publicitaires.

Seuls des cookies techniques strictement nécessaires au fonctionnement peuvent être utilisés (session, authentification).

---

## 11. Modifications de la Politique

Cette politique peut être mise à jour pour refléter les évolutions légales ou techniques.

En cas de modification substantielle, vous en serez informé via votre notaire.

| Version | Date | Modification |
|---------|------|--------------|
| 1.0 | 24/01/2026 | Version initiale |

---

## 12. Contact

Pour toute question relative à cette politique :

- **Votre notaire** : Premier interlocuteur pour votre dossier
- **DPO notariat** : dpo@adnov.fr
- **Support technique Notomai** : contact@notomai.fr

---

*Cette politique de confidentialité est établie conformément au Règlement (UE) 2016/679 relatif à la protection des données personnelles (RGPD) et à la loi n°78-17 du 6 janvier 1978 modifiée.*
